...
Int_t molana_burst_analysis(string FNAME, Int_t BURST = 10){
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
   ...
   TGraph * gr_asym_uncorr = new TGraph();
     gr_asym_uncorr->SetTitle( Form("BURST-LEVEL RAW ASYMMETRY : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_uncorr->SetMarkerStyle(6);
   TGraph * gr_asym_acccor = new TGraph();
     gr_asym_acccor->SetTitle( Form("BURST-LEVEL ACCIDENTAL-CORRECTED ASYMMETRY : RUN %i with BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_acccor->SetMarkerStyle(6);
   TGraph * gr_asym_bcmnrm = new TGraph();
     gr_asym_bcmnrm->SetTitle( Form("BURST-LEVEL RAW ASYMMETRY BCM NORMALIZED : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_bcmnrm->SetMarkerStyle(6);
   TGraph * gr_asym_cornor = new TGraph();
     gr_asym_cornor->SetTitle( Form("BURST-LEVEL ACCIDENTAL-CORRECTED ASYMMETRY BCM NORMALIZED : RUN %i with BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_cornor->SetMarkerStyle(6);
   TGraph * gr_asym_dtcorr = new TGraph();
     gr_asym_dtcorr->SetTitle( Form("BURST-LEVEL DEADTIME-ONLY CORRECTED ASYMMETRY : RUN %i with BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_dtcorr->SetMarkerStyle(6);
   TGraph * gr_asym_final = new TGraph();
     gr_asym_final->SetTitle( Form("BURST-LEVEL FULLY CORRECTED ASYMMETRY : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_final->SetMarkerStyle(6);

   TGraph * gr_asym_BCM = new TGraph();
     gr_asym_BCM->SetTitle( Form("BURST-LEVEL BCM ASMMETRY : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_BCM->SetMarkerStyle(6);
   TGraph * gr_asym_LEFT = new TGraph();
     gr_asym_LEFT->SetTitle( Form("BURST-LEVEL LEFT ASYMMETRY (DT RELEVANT) : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_LEFT->SetMarkerStyle(6);
   TGraph * gr_asym_ACCID = new TGraph();
     gr_asym_ACCID->SetTitle( Form("BURST-LEVEL ACCIDENTAL ASYMMETRY (ACCIDENTAL RELEVANT) : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_asym_ACCID->SetMarkerStyle(6);

   TGraph * gr_rate_LEFT = new TGraph();
     gr_rate_LEFT->SetTitle( Form("BURST-LEVEL LEFT RATE (DT RELEVANT) : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_rate_LEFT->SetMarkerStyle(6);
   TGraph * gr_rate_ACCID = new TGraph();
     gr_rate_ACCID->SetTitle( Form("BURST-LEVEL ACCID RATE (ACCIDENTALS RELEVANT) : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_rate_ACCID->SetMarkerStyle(6);
   TGraph * gr_rate_BCM = new TGraph();
     gr_rate_BCM->SetTitle( Form("BURST-LEVEL BCM RATE : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
     gr_rate_BCM->SetMarkerStyle(6);
   TGraph * gr_rate_COIN = new TGraph();
     gr_rate_COIN->SetTitle( Form("BURST-LEVEL COINCIDENCE RATE (ACCIDENTALS RELEVANT) : RUN %i WITH BURST SIZE %i;Local JLab Time; Asymmetry; Count",RUNN,BURST));
  ...
  Cgr_rate_left->SaveAs(   Form( "41__burst_analysis_left_rate.png", RUNN) );

  Cgr_rate_accid->SaveAs(  Form( "43__burst_analysis_accd_rate.png", RUNN) );
  Cgr_rate_coin->SaveAs(   Form( "44__burst_analysis_coin_rate.png", RUNN) );
  Cgr_rate_bcm->SaveAs(    Form( "45__burst_analysis_qbcm_rate.png", RUNN) );

  Cgr_asym_LEFT->SaveAs(   Form( "51__burst_analysis_left_sing_asym.png", RUNN) );

  Cgr_asym_ACCID->SaveAs(  Form( "53__burst_analysis_accidentl_asym.png", RUNN) );

  Cgr_asym_uncorr->SaveAs( Form( "61__burst_analysis_not_corrected_asym.png", RUNN) );
  Cgr_asym_acccor->SaveAs( Form( "62__burst_analysis_acc_corrected_asym.png", RUNN) );
  Cgr_asym_bcmnrm->SaveAs( Form( "63__burst_analysis_bcm_corrected_asym.png", RUNN) );
  Cgr_asym_cornor->SaveAs( Form( "64__burst_analysis_acc_and_qcorr_asym.png", RUNN) );
  Cgr_asym_dtcorr->SaveAs( Form( "65__burst_analysis_deadtime_corr_asym.png", RUNN) );
  Cgr_asym_final->SaveAs(  Form( "66__burst_analysis_fullcorrected_asym.png", RUNN) );
