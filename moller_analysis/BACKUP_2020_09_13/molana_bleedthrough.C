...
//BEAM: 0 --> OFF; BEAM:1 --> ON LOOKING FOR TRIPS
void molana_bleedthrough(string FILE, Double_t FREQ, Bool_t BEAM ){
  ...
  H[ 6]->Draw();
  c_beamon_bleed->cd(4)->SetLogy();
  H[ 7]->GetXaxis()->SetRangeUser(H[ 6]->FindFirstBinAbove( 0. , 1 )-sidebuff,H[ 6]->FindLastBinAbove ( 0. , 1 )+sidebuff);
  H[ 7]->Draw();
