...
Int_t molana_prompt_analysis(string FILE, Int_t HELN, Double_t FREQ, Double_t ANPOW, Double_t QPED, Double_t TAU ){
  ...
  TGraph * gr_charg = new TGraph();
    gr_charg->SetTitle( Form("Prompt BCM Scaler : Run %i;Entry",RUNN));
  ...
  for (Long64_t entry = 0; entry < nentries; entry++) {
    ...
    if( goodhelpatt > 0 ){ 
      ...
      //cout << "[" << fileeventnumber_stk[goodpattindex] << "] Good cycle coin recorded to patterns temp array: ";
      for(Int_t i = 0; i < heln; i++){
        if(temphels[i] == 0){
          //cout << tempcoin[i] << "/0, ";
          coin0 += tempcoin[i];
      ...
      //cout << "Bad qasymmetry: " << std::setw(11) << std::fixed << (curr1 - curr0) / (curr1 + curr0) << endl; 

      temppattvaluesI[ 0] = temptime; 
      temppattvaluesI[ 1] = tempnumb;         //numb
      temppattvaluesI[ 2] = tempsign;         //sign
      temppattvaluesI[ 3] = errorcode_stk[patterrindex];
      //what to do if cycle tossed beccause of bad neighbor?
      if(temppattvaluesI[3]==0) temppattvaluesI[3] = -1;
      //cout << "         Error code recorded for pattern: " << temppattvaluesI[3] << " = " << errorcode_stk[patterrindex] << endl;
  ...
  TCanvas * c_scalersums = new TCanvas("c_scalersums","c_scalersums",width_allplots,height_ratio_g);
  ...
  TCanvas * c_increment1 = new TCanvas("c_increment1","c_increment1",width_allplots,height_ratio_g);
  Int_t sidebuff = 100;
  ...
  TCanvas * c_increment2 = new TCanvas("c_increment2","c_increment2",width_allplots,height_ratio_g);
  ...
  TCanvas * c_increment3 = new TCanvas("c_increment3","c_increment3",width_allplots,height_ratio_s);
  ...
  TCanvas * c_asymhistos = new TCanvas("c_asymhistos","c_asymhistos",width_allplots,height_ratio_s);
  ...
  Double_t aavg = fit6->GetParameter(1);		//ASYMMETRY MEAN FROM GAUS FIT
  Double_t aerr = fit6->GetParError(1); 		//ASYMMETRY MEAN ERROR FROM GAUS FIT
  Double_t pavg = fitgrpolr->GetParameter(0);		//POLARIZATION MEAN FROM POL0 FIT
  Double_t perr = fitgrpolr->GetParError(0);		//POLARIZATION MEAN ERROR FROM POL0 FIT
  summary << std::fixed 
          << std::setw(5) << RUNN << "  "
