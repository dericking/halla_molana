...
Int_t molana_prompt_analysis(string FILE, Int_t HELN, Double_t FREQ, Double_t ANPOW, Double_t QPED, Double_t TAU ){
  ...
  const Double_t ptar       = 0.08005;         //TARGET POLARIZATION
  const Float_t  bcmped     = QPED;            //BCM PEDESTAL
  ...
  //CHANGED SOME VALUES TO FLOATS TO ACCOMODATE NON-INT CORRECTIONS SUCH AS QPED AND DEADTIME
  Float_t    singl1_stk[stksz];
  Float_t    singr1_stk[stksz];
  Float_t    coinc1_stk[stksz];
  Float_t    accid1_stk[stksz];
  Float_t    singl1_stk2[stksz]; //Explicitly for pattern tree to pass val unaltered for DT
  Float_t    singr1_stk2[stksz]; //Explicitly for pattern tree to pass val unaltered for DT
  Float_t    coinc1_stk2[stksz]; //Explicitly for pattern tree to pass val unaltered for DT
  Float_t    accid1_stk2[stksz]; //Explicitly for pattern tree to pass val unaltered for DT
  Float_t    bcm_stk[stksz];
  Int_t      ledflash_stk[stksz];
  Int_t      clockheltrig_stk[stksz];
  Int_t      mps_stk[stksz];
  Int_t      clockheldel_stk[stksz];
  Int_t      clocktimer_stk[stksz];
  Float_t    singl2_stk[stksz];
  Float_t    singr2_stk[stksz];
  Float_t    coinc2_stk[stksz];
  Float_t    accid2_stk[stksz];
  ...
  Int_t      trighelvalue_stk[stksz]; // Helicity 0 or 1
  Int_t      trig6_stk[stksz];
  Int_t      trighelpatt_stk[stksz];  // 0 on Pattern Start
  ...
  Int_t preverrorindex = 0;    //STORE PREVIOUS ERROR INDEX
  ...
  H[ 9] = new TH1F("incdiffleft", Form("Left Increments (Mod1-Mod2) : Run %i",RUNN), i_incrbin, i_incrmin, i_incrmax);
  H[10] = new TH1F("incdiffrght", Form("Right Increments (Mod1-Mod2) : Run %i",RUNN),i_incrbin, i_incrmin, i_incrmax);
  H[11] = new TH1F("incdiffcoin", Form("Coin Increments (Mod1-Mod2) : Run %i",RUNN), i_incrbin, i_incrmin, i_incrmax);
  H[12] = new TH1F("incdiffaccd", Form("Accid Increments (Mod1-Mod2) : Run %i",RUNN),i_incrbin, i_incrmin, i_incrmax);
  H[13] = new TH1F("incclktimer", Form("Clock Timer Increments : Run %i",RUNN),      i_incrbin, i_incrmin, i_incrmax);
  Int_t asymbin = 301;
  Int_t asymmin =  -1;
  Int_t asymmax =   1;
  H[14] = new TH1F("asym_uncr", Form("Prompt Pattern-Level Uncorrected Asym Dist : Run %i",RUNN), asymbin, asymmin, asymmax);
  H[15] = new TH1F("asym_corr", Form("Prompt Pattern-Level Corrected Asym Dist : Run %i",RUNN),   asymbin, asymmin, asymmax);
  H[16] = new TH1F("qasymhist", Form("Prompt Pattern-Level Charge Asymmetry Dist : Run %i",RUNN), asymbin, asymmin, asymmax);


  ///////////////////////////////////////////////////////////////  ⠂⠂⠂⠂⠂◘⠨⠨⠨⠨⠨⠨⠨⠨
  //GRAPHS USED FOR PLOTTING SCALERS
  TGraph * gr_sngl1 = new TGraph();
    gr_sngl1->SetTitle( Form("Prompt Left Singles Scaler : Run %i;Entry",RUNN));
  ...
  TGraph * gr_sngr1 = new TGraph();
    gr_sngr1->SetTitle( Form("Prompt Right Singles Scaler : Run %i;Entry",RUNN));
    gr_sngr1->SetMarkerStyle(6);
  TGraph * gr_coin1 = new TGraph();
    gr_coin1->SetTitle( Form("Prompt Coincidence Scaler (Module 1) : Run %i;Entry",RUNN));
    gr_coin1->SetMarkerStyle(6);
  TGraph * gr_accd1 = new TGraph();
    gr_accd1->SetTitle( Form("Prompt Accidental Scaler (Module 1) : Run %i;Entry",RUNN));
    gr_accd1->SetMarkerStyle(6);
  TGraph * gr_charg = new TGraph();
    gr_charg->SetTitle( Form("Prompt BCM Scaler (Not Ped Subt'd) : Run %i;Entry",RUNN));
    gr_charg->SetMarkerStyle(6);
  TGraph * gr_clock = new TGraph();
    gr_clock->SetTitle( Form("Prompt Clock Generator : Run %i;Entry",RUNN));
    gr_clock->SetMarkerStyle(6);
  TGraph * gr_ticks = new TGraph();
    gr_ticks->SetTitle( Form("Prompt Ticks : Run %i;Entry",RUNN));
    gr_ticks->SetMarkerStyle(6);

  TGraph * gr_bcmrt = new TGraph();
    gr_bcmrt->SetTitle( Form("Prompt Pattern-Level BCM Over Time : Run %i;Entry$",RUNN) );
    gr_bcmrt->SetMarkerStyle(6); // SCALER RATES FILLED BY HELICITY FLIP
  TGraph * gr_slrat = new TGraph();
    gr_slrat->SetTitle( Form("Prompt Pattern-Level Left Singles Rate : Run %i;Entry$",RUNN) );
    gr_slrat->SetMarkerStyle(6);
  TGraph * gr_srrat = new TGraph();
    gr_srrat->SetTitle( Form("Prompt Pattern-Level Right Singles Rate : Run %i;Entry$",RUNN) );
    gr_srrat->SetMarkerStyle(6);
  TGraph * gr_cnrat = new TGraph();
    gr_cnrat->SetTitle( Form("Prompt Pattern-Level Coincidence Rate : Run %i;Entry$",RUNN) );
    gr_cnrat->SetMarkerStyle(6);
  TGraph * gr_acrat = new TGraph();
    gr_acrat->SetTitle( Form("Prompt Pattern-Level Accidentals Rate : Run %i;Entry$",RUNN) );
    gr_acrat->SetMarkerStyle(6);
  TGraph * gr_clkrt = new TGraph();
    gr_clkrt->SetTitle( Form("Prompt Pattern-Level Clock over Time : Run %i;Entry$",RUNN) );
    gr_clkrt->SetMarkerStyle(6);

  TGraph * gr_asymm = new TGraph();
    gr_asymm->SetTitle( Form("Prompt Pattern-Level Corrected Asymmetry : Run %i;Entry$",RUNN) );
    gr_asymm->SetMarkerStyle(6); //FILLED BY PATTERN //FILLED BY HELICITY PATTERN
  TGraph * gr_qasym = new TGraph();
    gr_qasym->SetTitle( Form("Prompt Pattern-Level Charge Asymmetry : Run %i;Entry$",RUNN) );
    gr_qasym->SetMarkerStyle(6);
  TGraph * gr_polar = new TGraph();
    gr_polar->SetTitle( Form("Prompt Pattern-Level Polarizaton : Run %i;Entry$",RUNN) );
  ...
  const Int_t npattD = 21;
  const Int_t npattI = 6;
  Double_t    temppattvaluesD[npattD];
  Int_t       temppattvaluesI[npattI];
  Char_t pattnamesD[npattD][32] = {"coin0",    // D
                                   "coin1",    // D
                                   "coinavg",  // D
                                   "coinasymuncorr", // D
                                   "coinasym", // D
  ...
  for (Long64_t entry = 0; entry < nentries; entry++) {
    ...
    // A "■" BREAKS THE CYCLES/FLIPS INTO PATTERNS, 
    // A "◁□□□□▷" DENOTES HELICITY TRIGGER = 0 FOUND THUS NEW PATTERN WILL BE WRITTEN 
    if(printascii && trighelpatt == 0 && entry != 0){
      ...
      for(Int_t i = 0; i < stksz; i++){
        if(trighelpatt_stk[i]==0 && entry%stksz == i){
          ...
          output << " ■" << std::setw(6) << fileeventnumber_stk[i]%10000 << " ";
        }else if(trighelpatt_stk[i]!=0 && entry%stksz == i){
          output << std::setw(6) << " ◁□□□□▷";
        }else{
          output << std::setw(6) << fileeventnumber_stk[i]%10000 << " ";
      ...
      output << "]" << endl;
      //PRINT HELICITY STACK   trighelvalue_stk
      output << "helstack[";
      for(Int_t i = 0; i < stksz; i++){
        if(trighelpatt_stk[i]==0 && entry%stksz == i){
          output << "  " << std::setw(6) << "◁□□□□▷";
      ...
      output << "]" << endl;
      //PRINT COINC1 STACK   coinc1_stk
      output << "coinstk1[";
      for(Int_t i = 0; i < stksz; i++){
        if(trighelpatt_stk[i]==0 && entry%stksz == i){
          output << "  " << std::setw(6) << "◁□□□□▷";
        }else if(trighelpatt_stk[i]==0 && entry%stksz != i){
          output << " ■" << std::setw(6) << (Int_t)coinc1_stk[i] << " ";
        }else if(trighelpatt_stk[i]!=0 && entry%stksz == i){
          output << std::setw(6) << " ◁□□□□▷";
        }else{
          output << std::setw(6) << (Int_t)coinc1_stk[i] << " ";
      ...
      output << "]" << endl;
      //PRINT ACCIDENTAL STACK   accid1_stk
      output << "accdstk1[";
      for(Int_t i = 0; i < stksz; i++){
        if(trighelpatt_stk[i]==0 && entry%stksz == i){
          output << "  " << std::setw(6) << "◁□□□□▷";
        }else if(trighelpatt_stk[i]==0 && entry%stksz != i){
          output << " ■" << std::setw(6) << (Int_t)accid1_stk[i] << " ";
        }else if(trighelpatt_stk[i]!=0 && entry%stksz == i){
          output << std::setw(6) << " ◁□□□□▷";
        }else{
          output << std::setw(6) << (Int_t)accid1_stk[i] << " ";
      ...
      output << "]" << endl;
      //PRINT BCM STACK   bcm_stk
      output << "bcmstack[";
      for(Int_t i = 0; i < stksz; i++){
        if(trighelpatt_stk[i]==0 && entry%stksz == i){
          output << "  " << std::setw(6) << "◁□□□□▷";
        }else if(trighelpatt_stk[i]==0 && entry%stksz != i){
          output << " ■" << std::setw(6) << (Int_t)bcm_stk[i] << " ";
        }else if(trighelpatt_stk[i]!=0 && entry%stksz == i){
          output << std::setw(6) << " ◁□□□□▷";
        }else{
          output << std::setw(6) << (Int_t)bcm_stk[i] << " ";
      ...
      output << "]" << endl;
      //PRINT TICKS STACK   ticks_stk
      output << "tick_stk[";
      for(Int_t i = 0; i < stksz; i++){
        if(trighelpatt_stk[i]==0 && entry%stksz == i){
          output << "  " << std::setw(6) << "◁□□□□▷";
        }else if(trighelpatt_stk[i]==0 && entry%stksz != i){
          output << " ■" << std::setw(6) << (Int_t)ticks_stk[i] << " ";
        }else if(trighelpatt_stk[i]!=0 && entry%stksz == i){
          output << std::setw(6) << " ◁□□□□▷";
        }else{
          output << std::setw(6) << (Int_t)ticks_stk[i] << " ";
    ...
    Float_t dead_time_rate_use       = singl1 * freq;              //USING SINGLE LEFT FOR DEADTIME
    Float_t dead_time_corr_fac       = pow(euler,-1*dead_time_rate_use/1000*deadtimetau);

    singl1_stk[entry%stksz]          = singl1 / dead_time_corr_fac;
    singr1_stk[entry%stksz]          = singr1 / dead_time_corr_fac;
    coinc1_stk[entry%stksz]          = coinc1 / dead_time_corr_fac;
    accid1_stk[entry%stksz]          = accid1 / dead_time_corr_fac;

    singl1_stk2[entry%stksz]         = singl1; //explicitly used for making pattern file with unaltered values
    singr1_stk2[entry%stksz]         = singr1; //explicitly used for making pattern file with unaltered values
    coinc1_stk2[entry%stksz]         = coinc1; //explicitly used for making pattern file with unaltered values
    accid1_stk2[entry%stksz]         = accid1; //explicitly used for making pattern file with unaltered values

    if((bcm-bcmped)<2.) bcm_stk[entry%stksz] = 0;           //If this quantity is < 1. then beam is off.
    else                bcm_stk[entry%stksz] = bcm-bcmped;  //If this quantity is > 1. then beam is on; record ped subtracted value.
    ...
    bcmscalsm += bcm;                 //BCM SCALER SUM -- Scaler charts, do not sub bcm ped.
    ...
    if( trighelpatt == 0 ){
      ...
      preverrorindex = errorindex; //SAVE PREVIOUS INDEX.
      errorindex = entry%stksz;    //WHERE ON THE STACK WILL THIS ERROR CODE BE SAVED
      errorcode_stk[errorindex] = 0;//RESET THIS CODE TO ZERO ON THE STACK
      ...
      if( numhelcyc != heln ){
        ...
        errorcode_stk[errorindex] = pow(2,0);//|= (1 << 0);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 1" << endl;
      }
    ...
    if( (abs(singl1-singl2) > 2) ){          //DO LEFT SCALER COUNTS MATCH?
      ...
      errcnts[4]++;
      errorcode_stk[errorindex] = pow(2,4);//|= (1 << 4);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 4" << endl;
      if(printascii) output << "[" << entry << "] ERROR Left Scaler Issue" << endl;
    }
    if( (abs(singr1-singr2) > 2) ){          //DO RIGHT SCALER COUNTS MATCH?
      goodhelpatt = -1*skippatt;
      errcnts[5]++;
      errorcode_stk[errorindex] = pow(2,5);//|= (1 << 5);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 5" << endl;
      if(printascii) output << "[" << entry << "] ERROR Right Scaler Issue" << endl;
    }
    if( (abs(coinc1-coinc2) > 2) ){          //DO COINCIENCE SCALER COUNTS MATCH?
      goodhelpatt = -1*skippatt;
      errcnts[7]++;
      errorcode_stk[errorindex] = pow(2,7);//|= (1 << 7);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 7" << endl;
      if(printascii) output << "[" << entry << "] ERROR Coin Scaler Issue" << endl;
    }
    if( (abs(accid1-accid2) > (0.04*accid1)+2 ) ){          //DO ACCIDENTAL SCALER COUNTS MATCH?
      goodhelpatt = -1*skippatt;
      //cout << "accid1: " << accid1 << ", accid2: " << accid2 << ", diff\%: " << 100*abs(accid2-accid1)/(0.5*(accid2+accid1)) << "\%" << endl;
      errcnts[8]++;
      errorcode_stk[errorindex] = pow(2,8);//|= (1 << 8);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 8" << endl;
    ...
    if( accid1 > coinc1 ){                   //ACCID > COIN? ISSUE WITH LOW RATE SCANS
      ...
      errorcode_stk[errorindex] = pow(2,12);//|= (1 << 12);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 12" << endl;
    ...
    if( (clockheltrig != trighelvalue) || (clockheltrig < 0) ){
      goodhelpatt = -1*skippatt;
      errcnts[1]++;
      errorcode_stk[errorindex] = pow(2,1);//|= (1 << 1);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 1" << endl;
...
/*
    if( clocktimer < (expclock*0.95) || clocktimer > (expclock*1.05) ){//IS CLOCK INCREMENT ACCEPTABLE?
      goodhelpatt = -1*skippatt;
      errcnts[11]++;
      errorcode_stk[errorindex] = pow(2,11);//|= (1 << 11);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 11" << endl;
...
*/
    ...
    if( bcm < 0 ){                            //NEGATIVE INCREMENT SIGNIFIES A SCALAR PROBLEM
      goodhelpatt = -1*skippatt;
      errcnts[6]++;
      errorcode_stk[errorindex] = pow(2,6);//|= (1 << 6);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 6" << endl;
    ...
    if( bcm < 10 ){                           //
      goodhelpatt = -1*skippatt;
      errcnts[3]++;
      errorcode_stk[errorindex] = pow(2,3);//|= (1 << 3);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 3" << endl;
    ...
    if( goodhelpatt > 0 ){
      ...
      if( cyclecount != (heln-1) ){
        goodhelpatt = -1*skippatt;
        errcnts[2]++;
        errorcode_stk[patterrindex] += pow(2,0);//|= (1 << 2);
        //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 0" << endl;
      ...
      for(Int_t k = 1; k < heln; k++){
        ...
        if( ((k%2)+pow(0,abs(k%4))) != ((pattarray[k-1]+pattarray[k])%2) ){
          goodhelpatt = -1*skippatt;
          errcnts[2]++;
          errorcode_stk[patterrindex] += pow(2,2);//|= (1 << 2);
          //cout << "*ERROR* FOR PATTERN INDEX [" << errorindex << "]: 2" << endl;
    ...
    Int_t   tempindex(0);      //index of beginning of cycle in increments file
    ...
    if( goodhelpatt > 0 ){ 
      ...
      for(Int_t i = 0; i < heln; i++){
        ...
        tempcoin[i] = coinc1_stk2[goodpattindex]; //stack2: unaltered coins
        templeft[i] = singl1_stk2[goodpattindex]; //stack2: unaltered lefts
        temprght[i] = singr1_stk2[goodpattindex]; //stack2: unaltered right
        tempaccd[i] = accid1_stk2[goodpattindex]; //stack2: unaltered accid
        ...
        if(i==0) temperror = 0;
        //if(i==0) tempindex = goodpattindex;  
      ...
      if( !(::isnan(asymu) || ::isnan(asymc) || ::isnan(qasym) ) ){
        ...
        //cout << "Prompted qasymmetry: " << std::setw(11) << std::fixed << qasym;
      ...
      Double_t coin0(0.),coin1(0.),coinS(0.),left0(0.),left1(0.),leftS(0.);
      Double_t rght0(0.),rght1(0.),rghtS(0.),accd0(0.),accd1(0.),accdS(0.);
      Double_t curr0(0.),curr1(0.),currS(0.);
      for(Int_t i = 0; i < heln; i++){
        if(temphels[i] == 0){
          coin0 += tempcoin[i];
          left0 += templeft[i];
      ...
      temppattvaluesD[ 0] = coin0;// / heln;
      temppattvaluesD[ 1] = coin1;// / heln;
      temppattvaluesD[ 2] = (coin1 + coin0);// / heln;
      temppattvaluesD[ 3] = ( ((coin1 - 0.)/curr1) - ((coin0 - 0.)/curr0) ) / ( ((coin1 - 0.)/curr1) + ((coin0 - 0.)/curr0) );
      temppattvaluesD[ 4] = ( ((coin1 - accd1)/curr1) - ((coin0 - accd0)/curr0) ) / ( ((coin1 - accd1)/curr1) + ((coin0 - accd0)/curr0) );

      temppattvaluesD[ 5] = left0;// / heln;
      temppattvaluesD[ 6] = left1;// / heln;
      temppattvaluesD[ 7] = (left1 + left0);// / heln; 
      temppattvaluesD[ 8] = (left1 - left0) / (left1 + left0); 

      temppattvaluesD[ 9] = rght0;// / heln;
      temppattvaluesD[10] = rght1;// / heln;
      temppattvaluesD[11] = (rght1 + rght0);// / heln; 
      temppattvaluesD[12] = (rght1 - rght0) / (rght1 + rght0); 

      temppattvaluesD[13] = accd0;// / heln;
      temppattvaluesD[14] = accd1;// / heln;
      temppattvaluesD[15] = (accd1 + accd0);// / heln; 
      temppattvaluesD[16] = (accd1 - accd0) / (accd1 + accd0); 

      temppattvaluesD[17] = curr0;// / heln;
      temppattvaluesD[18] = curr1;// / heln;
      temppattvaluesD[19] = (curr1 + curr0);// / heln; 
      temppattvaluesD[20] = (curr1 - curr0) / (curr1 + curr0); 

      //cout << ", Patt qasym:" << std::setw(11) << std::fixed << (curr1 - curr0) / (curr1 + curr0) << endl; 


      //THIS IS NO LONGER NECESSARY AS THE ERROR BUG HAS BEEN RESOLVED.
      //if( std::isinf(temppattvaluesD[4]) || std::isnan(temppattvaluesD[4]) ) 
      //cout << ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>PROBLEM! @" << tempnumb << endl;
      ...
      temppattvaluesI[ 4] = RUNN;
      //temppattvaluesI[ 5] = tempindex;
      //cout << "trigger(" << trighelpatt_stk[patterrindex] << ") errorcode(" << errorcode_stk[patterrindex] << ")" << endl;


      output << "^^^^ heltrig(" << trighelpatt_stk[patterrindex] << ") entry: " << fileeventnumber_stk[patterrindex] << ", errorcode(" << temppattvaluesI[ 3] << ")" << endl << endl;
      trPatt->Fill();
      ...
      //What to print about cycle skipped ?  Error index...
      //cout << "Skipped pattern index in prompt analysis: " << preverrorindex << "<<<<<" << endl;
      //cout << "          Error code to prev error index: " << errorcode_stk[preverrorindex] << endl;

      //GET THE INDEX FOR THE GOOT PATERN AND START CYCLING
      Int_t goodpattindex = (entry-1*heln)%stksz;   //INDEX TO START AT FOR HELICITY STACK
      if(goodpattindex < 0) goodpattindex += stksz; //WHY DOES C++ SOMETIMES RETURN NEGATIVE MODULO, OR IS % A REMAINDER HERE???
      Int_t patterrindex = goodpattindex;
      //cout << "          Error pattern index identified: " << patterrindex << "  should match {" << preverrorindex << "}" << endl;
      for(Int_t i = 0; i < heln; i++){
        Int_t helstate = trighelvalue_stk[ goodpattindex ];
        ...
        //RECORD THE INCREMENTS IN THE TEMPORARY STACKS USED FOR THE PATTERN TREE
        temphels[i] = helstate;                  //helicity sign
        tempcoin[i] = coinc2_stk[goodpattindex]; //coin
        templeft[i] = singl2_stk[goodpattindex]; //left
        temprght[i] = singr2_stk[goodpattindex]; //right
        tempaccd[i] = accid2_stk[goodpattindex]; //accid
        tempcurr[i] = bcm_stk[goodpattindex];    //bcm
      ...
      Double_t rght0(0.),rght1(0.),rghtS(0.),accd0(0.),accd1(0.),accdS(0.);
      Double_t curr0(0.),curr1(0.),currS(0.);
      //cout << "[" << fileeventnumber_stk[goodpattindex] << "] BADD cycle coin recorded to patterns temp array: ";
      for(Int_t i = 0; i < heln; i++){
        if(temphels[i] == 0){
          coin0 += tempcoin[i];
          left0 += templeft[i];
      ...
      temppattvaluesI[ 2] = tempsign;         //sign
      temppattvaluesI[ 3] = errorcode_stk[patterrindex];
      if(temppattvaluesI[3]==0) temppattvaluesI[3] = -1; //If this cycle gets tossed it's an adjacency cut.
      temppattvaluesI[ 4] = RUNN;
      temppattvaluesI[ 5] = tempindex;
  ...
  errorsummary << "Run " << RUNN << " Reasons/Errors for Cycles Discarded: " << endl;
  errorsummary << "  Total root file entries: " << nentries << endl;
  errorsummary << "     Index      Code                         Error Name               Count" << endl;
  for(Int_t j = 0; j <= ne; j++){
    if( j >= ne ) break;  //SAFETY STATEMENT, WAS CROSSING 'ne' FOR SOME REASON.
    errorsummary << "  " << std::setw(8)  << j
                 << "  " << std::setw(8)  << pow(2,j) 
                 << "  " << std::setw(45) << (*errname)[j] 
                 << "  " << std::setw(6)  << errcnts[j] 
                 << endl;
  }
  errorsummary.close();

  //////////////////////////////////////////////////////////  ⠂⠂⠂⠂⠂◘⠨⠨⠨⠨⠨⠨⠨⠨
  //Close the patterns file here. THERE ARE EXIT COMMANDS AFTER THIS. DO NOT PLACE AFTER THIS.
  fout->Write();
  ...
  TCanvas * c_scalersums = new TCanvas("c_scalersums","c_scalersums",width_allplots,height_ratio_s);
  ...
  TCanvas * c_increment1 = new TCanvas("c_increment1","c_increment1",width_allplots,height_ratio_s);
  Int_t sidebuff = 100;
  c_increment1->Divide(3,2);
  c_increment1->cd(1)->SetLogy();
  H[ 0]->GetXaxis()->SetRangeUser((H[ 0]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 0]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 0]->Draw();
  c_increment1->cd(2)->SetLogy();
  H[ 5]->GetXaxis()->SetRangeUser((H[ 5]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 5]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 5]->Draw();
  c_increment1->cd(3)->SetLogy();
  H[ 9]->GetXaxis()->SetRangeUser((H[ 9]->FindFirstBinAbove( 0. , 1 )-sidebuff/10)*i_incrmax/i_incrbin,(H[ 9]->FindLastBinAbove ( 0. , 1 )+sidebuff/10)*i_incrmax/i_incrbin);
  H[ 9]->Draw();
  c_increment1->cd(4)->SetLogy();
  H[ 1]->GetXaxis()->SetRangeUser((H[ 1]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 1]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 1]->Draw();
  c_increment1->cd(5)->SetLogy();
  H[ 6]->GetXaxis()->SetRangeUser((H[ 6]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 6]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 6]->Draw();
  c_increment1->cd(6)->SetLogy();
  H[10]->GetXaxis()->SetRangeUser((H[10]->FindFirstBinAbove( 0. , 1 )-sidebuff/10)*i_incrmax/i_incrbin,(H[10]->FindLastBinAbove ( 0. , 1 )+sidebuff/10)*i_incrmax/i_incrbin);
  H[10]->Draw();

  TCanvas * c_increment2 = new TCanvas("c_increment2","c_increment2",width_allplots,height_ratio_s);
  c_increment2->Divide(3,2);
  c_increment2->cd(1)->SetLogy();
  H[ 2]->GetXaxis()->SetRangeUser((H[ 2]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 2]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 2]->Draw();
  c_increment2->cd(2)->SetLogy();
  H[ 7]->GetXaxis()->SetRangeUser((H[ 7]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 7]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 7]->Draw();
  c_increment2->cd(3)->SetLogy();
  H[11]->GetXaxis()->SetRangeUser((H[11]->FindFirstBinAbove( 0. , 1 )-sidebuff/10)*i_incrmax/i_incrbin,(H[11]->FindLastBinAbove ( 0. , 1 )+sidebuff/10)*i_incrmax/i_incrbin);
  H[11]->Draw();
  c_increment2->cd(4)->SetLogy();
  H[ 3]->GetXaxis()->SetRangeUser((H[ 3]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 3]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 3]->Draw();
  c_increment2->cd(5)->SetLogy();
  H[ 8]->GetXaxis()->SetRangeUser((H[ 8]->FindFirstBinAbove( 0. , 1 )-sidebuff)*i_incrmax/i_incrbin,(H[ 8]->FindLastBinAbove ( 0. , 1 )+sidebuff)*i_incrmax/i_incrbin);
  H[ 8]->Draw();
  c_increment2->cd(6)->SetLogy();
  H[12]->GetXaxis()->SetRangeUser((H[12]->FindFirstBinAbove( 0. , 1 )-sidebuff/10)*i_incrmax/i_incrbin,(H[12]->FindLastBinAbove ( 0. , 1 )+sidebuff/10)*i_incrmax/i_incrbin);
  H[12]->Draw();

  TCanvas * c_increment3 = new TCanvas("c_increment3","c_increment3",width_allplots,height_ratio_b);
  ...
  TCanvas * c_asymhistos = new TCanvas("c_asymhistos","c_asymhistos",width_allplots,height_ratio_b);
  Double_t bufffact = 0.1;
  c_asymhistos->Divide(3,1);
  c_asymhistos->cd(1);
  H[14]->Draw();
  H[14]->Fit("gaus");
  ...
  H[16]->Draw();
  H[16]->Fit("gaus");
  H[16]->GetXaxis()->SetRangeUser( (((Double_t)asymmax-(Double_t)asymmin)/(Double_t)asymbin*(Double_t)H[16]->FindFirstBinAbove(0,1)*(1-bufffact)+(Double_t)asymmin) ,
                                   (((Double_t)asymmax-(Double_t)asymmin)/(Double_t)asymbin*(Double_t)H[16]->FindLastBinAbove (0,1)*(1+bufffact)+(Double_t)asymmin) );
  H[16]->Draw();


  TCanvas * c_grasymmtry = new TCanvas("c_grasymmtry","c_grasymmtry",width_allplots,height_ratio_b);
  gr_asymm->Draw("AP");
  gr_asymm->Fit("pol0");
  TF1 * fitgrasym = gr_asymm->GetFunction("pol0");
  gr_asymm->Draw("AP");

  TCanvas * c_grpolarizn = new TCanvas("c_grpolarizn","c_grpolarizn",1500,height_ratio_b);
  gr_polar->Draw("AP");
  gr_polar->Fit("pol0");
  TF1 * fitgrpolr = gr_polar->GetFunction("pol0");
  gr_polar->Draw("AP");

  TCanvas * c_grcoinrate = new TCanvas("c_grcoinrate","c_grcoinrate",1500,height_ratio_b);
  gr_cnrat->Draw("AP");
  gr_cnrat->Fit("pol0");
  TF1 * fitgrcrat = gr_cnrat->GetFunction("pol0");
  gr_cnrat->Draw("AP");

  TCanvas * c_grleftrate = new TCanvas("c_grleftrate","c_grleftrate",width_allplots,height_ratio_b);
  gr_slrat->Draw("AP");
  gr_slrat->Fit("pol0");
  TF1 * fitgrlrat = gr_slrat->GetFunction("pol0");
  gr_slrat->Draw("AP");

  TCanvas * c_grrghtrate = new TCanvas("c_grrghtrate","c_grrghtrate",width_allplots,height_ratio_b);
  gr_srrat->Draw("AP");
  gr_srrat->Fit("pol0");
  TF1 * fitgrrrat = gr_srrat->GetFunction("pol0");
  gr_srrat->Draw("AP");

  TCanvas * c_graccdrate = new TCanvas("c_graccdrate","c_graccdrate",width_allplots,height_ratio_b);
  gr_acrat->Draw("AP");
  gr_acrat->Fit("pol0");
  TF1 * fitgrarat = gr_acrat->GetFunction("pol0");
  gr_acrat->Draw("AP");

  TCanvas * c_grchrgasym = new TCanvas("c_grchrgasym","c_grchrgasym",width_allplots,height_ratio_b);
  gr_qasym->Draw("AP");
  gr_qasym->Fit("pol0");
  TF1 * fitgrqasm = gr_qasym->GetFunction("pol0");
  gr_qasym->Draw("AP");

  TCanvas * c_grchrgrate = new TCanvas("c_grchrgrate", "c_grchrgrate", width_allplots,height_ratio_b);
  gr_bcmrt->Draw("AP");
  gr_bcmrt->Fit("pol0");
  TF1 * fitgrqrat = gr_bcmrt->GetFunction("pol0");
  gr_bcmrt->Draw("AP");

  TCanvas * c_grclockgen = new TCanvas("c_grclockgen", "c_grclockgen", width_allplots,height_ratio_b);
  gr_clkrt->Draw("AP");
  gr_clkrt->Fit("pol0");
  ...
  summary << std::fixed 
          ...
          << std::setw(6) << qrat << "  "
          << std::setw(6) << clrt << "  "
          << std::setw(10) << std::setprecision(7) << aavg << "  "
          << std::setw(10) << std::setprecision(7) << aerr << "  "
  ...
  c_increment3->SaveAs( Form( "03_pattern_analysis_%i_increments3_histos.png", RUNN) );
  c_grleftrate->SaveAs( Form( "04_pattern_analysis_%i_left_singles_rates.png", RUNN) );
  c_grrghtrate->SaveAs( Form( "05_pattern_analysis_%i_right_singles_rate.png", RUNN) );
  c_grcoinrate->SaveAs( Form( "06_pattern_analysis_%i_coin_rate_over_run.png", RUNN) );
  c_graccdrate->SaveAs( Form( "07_pattern_analysis_%i_accd_rate_over_run.png", RUNN) );
  c_grclockgen->SaveAs( Form( "08_pattern_analysis_%i_clock_against_time.png", RUNN) );
  c_grchrgrate->SaveAs( Form( "09_pattern_analysis_%i_bcm_rate_over_time.png", RUNN) );
  c_grchrgasym->SaveAs( Form( "10_pattern_analysis_%i_bcm_asymmetry_time.png", RUNN) );
  c_asymhistos->SaveAs( Form( "11_pattern_analysis_%i_asymm_hists_fitted.png", RUNN) );
  c_grasymmtry->SaveAs( Form( "12_pattern_analysis_%i_asymm_over_time_gr.png", RUNN) );
  c_grpolarizn->SaveAs( Form( "13_pattern_analysis_%i_pol_over_time_grph.png", RUNN) );
