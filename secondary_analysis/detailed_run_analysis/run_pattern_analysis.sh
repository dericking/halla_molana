...
if [[ -z "${1}" ]]; then
    ...
    mysql -h ${MDBHOST} --user="${MDBUSER}" --password="${MDBPASS}" --database="${MDBNAME}" --skip-column-names -e "SELECT id_rundet FROM moller_run_details WHERE rundet_type LIKE 'beam_pol%';" > BURSTANALYSIS.txt    
else
...
while IFS=: read -r RUNNUMB; do
    ...
    #if [[ ${RUNNUMB} < 17094 ]]; then 
    if [[ ${RUNNUMB} < 19494 ]]; then 
